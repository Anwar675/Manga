...
export const MangaInfor = ({ category, manga, chapters }: MangaInforProps) => {
  ...
  const isFollow = followStatus?.isFollowed ?? false;

  const followMutation = useMutation(
    trpc.magas.followManga.mutationOptions({
      onSuccess: async () => {
        await Promise.all([
          queryClient.invalidateQueries(
            trpc.magas.getFollowStatus.queryFilter({
              mangaId: manga.id,
            }),
          ),
          queryClient.invalidateQueries(
  ...
  const unfollowMutation = useMutation(
    trpc.magas.unfollowManga.mutationOptions({
      onSuccess: async () => {
        await Promise.all([
          queryClient.invalidateQueries(
            trpc.magas.getFollowStatus.queryFilter({
  ...
  const handleToggleFollow = () => {
    if (isFollow) {
      ...
      followMutation.mutate({ mangaId: manga.id });
    }
  };
        ...
        <div className="bg-rank flex md:flex-row flex-col md:gap-2 gap-4 rounded-xl w-full p-4">
          <div className="text-center">
            ...
            <div className="p-4 flex gap-4 justify-around items-center">
              ...
              >
                <Bookmark className={isFollow ? "fill-current" : ""} />
            ...
            <div className="flex md:flex-row flex-col pr-8 justify-between">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-1">
                  ...
                  <p className="px-2 rounded-2xl bg-kind">
