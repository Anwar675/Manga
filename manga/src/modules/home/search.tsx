  ...
  return (
    <div className="relative w-full">
      ...
      {!value ? (
        ...
        <XIcon
          onClick={() => setValue("")}
          className="absolute top-1/2 -translate-y-1/2 right-3 cursor-pointer opacity-60 hover:opacity-100"
        />
      )}

      {debounced.trim() && mangas.length > 0 && (
        <div className="absolute bg-popular md:top-14 top-10 z-20 left-0 w-full md:w-80 rounded-sm max-h-96 overflow-auto shadow-lg">
          {mangas.map((manga) => (
            <Link
              key={manga.id}
            ...
            >
              <div className="flex cursor-pointer hover:bg-gray-600/10 p-2 gap-4">
                ...
                <div>
                  <h4 className="text-[18px] font-medium line-clamp-1">
                    {manga.title}
                  </h4>
